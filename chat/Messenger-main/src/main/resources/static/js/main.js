...
var chatBoxUser = document.querySelector('#chatbox-user');

var privateMessageInput = document.querySelector('#private-message');
var privateMessageId = document.querySelector('#private-message-id');
...
function connect(event) {
	...
	// Setting chatbox title
	chatBoxUser.innerText = username + "'s";
...
function onConnected() {
	...
	stompClient.subscribe('/user/topic/private-message', onMessageReceived);

    // Tell your username to the server
    stompClient.send("/app/chat.register",
        {},
        JSON.stringify({sender: username, type: 'JOIN', group: groupName})
    )
...
function sendPrivate(event) {
    ...
    if(messageContent && stompClient) {
        var chatMessage = {
            sender: username,
			receiver: privateMessageId.value,
...
privateMessageForm.addEventListener('submit', sendPrivate, true)
